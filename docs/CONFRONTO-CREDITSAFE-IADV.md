...
**bilanci storici**
(dieci solo nella scheda di ricerca), **57 indicatori** (contati a schermo), **codice SDI** (letto e
...
| Analisi su **ogni sede** aziendale                                          | **sì**               | Sismica **comunale** (dataset Protezione Civile maggio 2025). Idraulica **puntuale** ISPRA sulle coordinate (P3/P2/P1); senza coordinate resta il ripiego provinciale / «non determinata».                                                                                                                                                                                                                                                                                                                                |
...
| Report personalizzato con il proprio marchio                                | **sì, se compilato** | logo, denominazione e numero RUI in testata — **a condizione che l'anagrafica dello studio sia stata compilata**. Se manca, il documento esce comunque — uno che non si apre è un lavoro perso — ma sopra al report compare un rilievo che nomina esattamente cosa manca e dove compilarlo, perché è ciò che l'art. 58 del Reg. IVASS 40/2018 chiede sui documenti consegnati al contraente. Il rilievo sta **a schermo e non si stampa**: è un avviso per chi consegna, non una riga del documento che il cliente riceve |
...
| **Serie storica georiferita a 10 anni** (pioggia, grandine, vento, fulmini) | **parziale**         | pioggia, raffiche e **temporali** (weather_code WMO ≥ 95) sì; grandine e fulmini **no** (nessun archivio pubblico gratuito Italia), e il report lo dichiara. Spento di default: uso commerciale a pagamento                                                                                                                                                                                                                                                                                                               |
...
che questo documento riportava veniva dalla descrizione commerciale del servizio `IT-full`
e non corrispondeva a niente di misurabile nel prodotto. Va tenuta distinta da un'altra
cifra che le somiglia e non c'entra: il **prezzo** del servizio, che il listino pubblico
dichiara a 0,30 € ed è il valore che il prodotto usa davvero (`config.ts`,
`costoCentesimi: 30`). Quarantotto non è né il numero degli indicatori né il prezzo.
...
### Le voci del loro report ancora aperte

- **Grandine e fulmini nello storico meteo.** Pioggia, raffiche e temporali (codice WMO) ci sono. Grandine e fulminazioni restano dichiarate non coperte: nessun archivio pubblico gratuito con copertura italiana affidabile. Servono LAMPINET / fonti a pagamento.
- ~~Sismica/idraulica per sede~~ — **chiuso**: sismica comunale PC maggio 2025; idraulica puntuale ISPRA sulle coordinate (`IDRAULICA_ISPRA=spento` per disattivare).
- ~~Valori di ricostruzione / immagini~~ — già fatte in precedenza.
