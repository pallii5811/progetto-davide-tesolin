...
CREATE POLICY utenti_isolamento_tenant ON utenti
  USING (tenant_id = current_setting('app.tenant_id', true)::uuid OR current_setting('app.ambito', true) = 'piattaforma')
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::uuid OR current_setting('app.ambito', true) = 'piattaforma');--> statement-breakpoint
...
CREATE POLICY registro_costi_dati_isolamento_tenant ON registro_costi_dati
  USING (tenant_id = current_setting('app.tenant_id', true)::uuid OR current_setting('app.ambito', true) = 'piattaforma')
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::uuid OR current_setting('app.ambito', true) = 'piattaforma');
