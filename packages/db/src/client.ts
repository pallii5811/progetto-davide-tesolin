...
const DDL: readonly string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS analisi (
    ...
    property_risk numeric CHECK (property_risk IS NULL OR (property_risk >= 0 AND property_risk <= 7)),
    bi_punteggio numeric CHECK (bi_punteggio IS NULL OR (bi_punteggio >= 0 AND bi_punteggio <= 7)),
    bi_perdita_giornaliera_centesimi bigint CHECK (bi_perdita_giornaliera_centesimi IS NULL OR bi_perdita_giornaliera_centesimi >= 0),
    cyber_risk numeric CHECK (cyber_risk IS NULL OR (cyber_risk >= 0 AND cyber_risk <= 7)),
